...
public class SparkV2Service extends SparkService {

    private static final Logger logger = Logger.getLogger(SparkV2Service.class.getName());
    ...
    public SparkV2Service(String server, String token, String roomIds) {
        this.server = server;
        this.token = token;
            ...
            try {
                ...
                if (responseCode != HttpStatus.SC_NO_CONTENT) {
                    if (logger.isLoggable(Level.WARNING)) {
                        logger.log(Level.WARNING, "Spark post may have failed. ResponseCode: {0}, Response: {1}",
                ...
                logger.log(Level.WARNING, "An IO error occurred while posting Spark notification", ioe);
            } finally {
                if (post != null) {
